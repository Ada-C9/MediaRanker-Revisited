...
describe Vote do
  ...
  describe "validations" do
    ...
    it "allows one user to vote for multiple works" do
      vote1 = Vote.new(user: users(:kari), work: work1)
      vote1.save!
      vote2 = Vote.new(user: users(:kari), work: work2)
    ...
    it "allows multiple users to vote for a work" do
      vote1 = Vote.new(user: users(:kari), work: work1)
      vote1.save!
      vote2 = Vote.new(user: users(:dan), work: work1)
      vote2.valid?.must_equal true
    end
    ...
    it "doesn't allow the same user to vote for the same work twice" do
      vote1 = Vote.new(user: users(:kari), work: work1)
      vote1.save!
      vote2 = Vote.new(user: users(:kari), work: work1)
